...
Image.MAX_IMAGE_PIXELS = None  # disable decompression bomb check for large dataset images
...
class RLHFDataset(Dataset):
    ...
    async def process_vision_info(
        ...
        # Inject data.max_pixels into each image dict so qwen_vl_utils.fetch_image
        # smart_resizes (it reads ele["max_pixels"] in vision_process.py:132). Without
        # this, the fallback IMAGE_MAX_TOKEN_NUM*factor**2 (~16M px) lets large val
        # figures pass through, producing prompts that exceed max_prompt_length and
        # crash agent_loop._postprocess.torch.cat (e.g. MMMU row 77 → 4666 tokens).
        max_pixels = None
            ...
            for msg in messages:
                ...
                for item in content:
                    if isinstance(item, dict) and item.get("type") == "image":
                        item.setdefault("max_pixels", int(max_pixels))
    ...
    def _process_multi_modal_info(
        ...
        if has_visual:
            ...
            # Inject data.max_pixels into each image dict so qwen_vl_utils.fetch_image
            # smart_resizes (mirrors the async process_vision_info path above). Without
            # this, AgentLoop's process_multi_modal_info path lets large images expand
            # past max_prompt_length and crash agent_loop._postprocess.torch.cat
            # (memory: v28_max_pixels_root_cause; f06/f08 hit this with 6878/8513 outliers).
            max_pixels = None
            if config is not None:
                max_pixels = config.get("max_pixels", None)
            if max_pixels is not None:
    ...
    async def process_multi_modal_info(
        ...
        # Video decode is awaited through ``rvrl_decode.decode_multi_modal_info``
        # so it does not block the AgentLoopWorker's event loop. Every knob
        # there is off by default, in which case this is the original inline
        # call.
        from verl.utils.dataset.rvrl_decode import decode_multi_modal_info

        return await decode_multi_modal_info(
            messages, image_patch_size, config, cls._process_multi_modal_info
        )
