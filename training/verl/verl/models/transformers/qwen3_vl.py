...
def _get_input_embeds(
    ...
    if pixel_values is not None:
        ...
        if n_image_tokens != n_image_features:
            # Off-by-N from smart_resize boundary disagreement between the two
            # rollout-side image-processor passes (verl#1640, #3469). Pad or
            # truncate visual features so masked_scatter doesn't crash; the row
            # contributes slightly noisy embeddings for the off-by-N patches
            # but the rest of the packed micro-batch trains normally.
            logger.warning(
                "qwen3_vl image token/feature mismatch: tokens=%d, features=%d. "
                "Padding/truncating image_embeds to match.",
                n_image_tokens, n_image_features,
            )
            if n_image_features < n_image_tokens:
                pad = image_embeds.new_zeros(n_image_tokens - n_image_features, image_embeds.shape[1])
                image_embeds = torch.cat([image_embeds, pad], dim=0)
                if deepstack_image_embeds is not None:
                    deepstack_image_embeds = [
                        torch.cat([emb, emb.new_zeros(n_image_tokens - emb.shape[0], emb.shape[1])], dim=0)
    ...
    if pixel_values_videos is not None:
        ...
        if n_video_tokens != n_video_features:
            # Symmetric off-by-N guard for video tokens (see image branch above).
            logger.warning(
                "qwen3_vl video token/feature mismatch: tokens=%d, features=%d. "
                "Padding/truncating video_embeds to match.",
                n_video_tokens, n_video_features,
            )
            if n_video_features < n_video_tokens:
                pad = video_embeds.new_zeros(n_video_tokens - n_video_features, video_embeds.shape[1])
                video_embeds = torch.cat([video_embeds, pad], dim=0)
                if deepstack_video_embeds is not None:
                    deepstack_video_embeds = [
                        torch.cat([emb, emb.new_zeros(n_video_tokens - emb.shape[0], emb.shape[1])], dim=0)
                        for emb in deepstack_video_embeds
                    ]
            else:
                video_embeds = video_embeds[:n_video_tokens]
                if deepstack_video_embeds is not None:
                    deepstack_video_embeds = [emb[:n_video_tokens] for emb in deepstack_video_embeds]
