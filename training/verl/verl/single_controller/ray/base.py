...
class ResourcePoolManager:
    ...
    def _check_resource_available(self):
        ...
        Uses cluster_resources() (total capacity) rather than
        available_resources_per_node() (currently free). The available view
        races during parallel rollout-replica init when the cluster is at
        exact capacity: by the time replica N's check runs, replicas 0..N-1
        have already claimed their PGs, leaving 0 free even though the
        cluster has enough total room. Ray's placement_group + pg.ready()
        path serializes the actual claims correctly, so the pre-flight only
        needs to assert total capacity.
        """
        cluster_resources = ray.cluster_resources()
        total_capacity_gpus = cluster_resources.get("GPU", 0) or cluster_resources.get("NPU", 0)
        total_required_gpus = sum(
            [n_gpus for process_on_nodes in self.resource_pool_spec.values() for n_gpus in process_on_nodes]
        )
        if total_capacity_gpus < total_required_gpus:
            raise ValueError(
                f"Total cluster GPUs {total_capacity_gpus} is less than total desired GPUs {total_required_gpus}"
            )
