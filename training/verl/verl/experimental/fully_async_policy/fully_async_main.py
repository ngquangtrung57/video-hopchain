...
class FullyAsyncTaskRunner:
    ...
    def _run_training_loop(self):
        ...
        # val_only mode: validation already ran via val_before_train above
        # (fully_async_main:113-114). Skip the training loop entirely.
        if self.components["config"].trainer.get("val_only", False):
            print("[ASYNC MAIN] val_only=True — skipping training loop, exiting")
            return
