...
class FullyAsyncRollouter(SeparateRayPPOTrainer):
    ...
    async def set_max_required_samples(self):
        async with self.lock:
            ...
            # Per-replica concurrent-sample cap, read from
            # VERL_MAX_CONCURRENT_PER_REPLICA and defaulting to 16.
            _per_replica_cap = int(os.environ.get("VERL_MAX_CONCURRENT_PER_REPLICA", "16"))
            self.max_concurrent_samples = len(self.llm_server_manager.get_replicas()) * _per_replica_cap
    ...
    async def get_exploration_stats(self) -> dict:
        """Aggregate entropy-aware exploration LP counters across all vLLM
        HTTP servers in this rollouter. Trainer calls this to populate the
        ``rvrl/lp_*`` W&B keys without needing direct access to the rollouter's
        AgentLoopManager (which only the rollouter side keeps).

        Returns a dict of summed counters across servers (or ``{}`` if no
        servers / exploration not enabled). Safe to call regardless of
        exploration state.
        """
        try:
            ...
            if mgr is None:
                return {}
            servers = getattr(mgr, "server_handles", None) or []
            if not servers:
                return {}
    ...
    async def _feed_samples(self):
        ...
        for epoch, batch_dict in continuous_iterator:
            ...
            # Monotonic per-prompt ordinal, consumed by the two-wave duty-cycle toggle.
            # Deliberately NOT meta_info["global_steps"]: on the other pipelines that key is the
            # TRAINING step, and trajectory_info["step"] must keep that meaning. self.global_steps
            # here increments once per prompt fed, never resets across epochs, and is rescaled
            # deterministically on resume, which is exactly what a duty-cycle toggle needs.
            full_batch.meta_info["rvrl_prompt_ordinal"] = int(self.global_steps)
