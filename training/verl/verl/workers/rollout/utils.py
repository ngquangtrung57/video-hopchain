...
def qwen2_5_vl_dedup_image_tokens(prompt_ids: list[int], processor):
    ...
    For Qwen3-VL, the HF processor pre-expands videos into per-frame blocks (one
    <vision_start>...<vision_end> span per frame, with timestamps in between).
    Plain consecutive-pad dedup leaves K stray <vision_start> <pad> <vision_end>
    blocks (one per frame) — vLLM's PromptReplacement then expands only the FIRST
    block per video item, leaving K-1 stray <video_pad> tokens whose mask exceeds
    the vision encoder output (masked_scatter assertion in qwen3_vl forward).

    For Qwen3-VL we therefore collapse ALL per-frame video blocks into a SINGLE
    <vision_start><video_pad><vision_end> per video so vLLM's single replacement
    matches exactly. This currently assumes one video per prompt.
    """
    if processor is None or not hasattr(processor, "image_processor"):
        return prompt_ids
    if "Qwen2VLImageProcessor" not in processor.image_processor.__class__.__name__:
        return prompt_ids

    is_qwen3vl = "Qwen3VL" in processor.__class__.__name__
    arr = np.array(prompt_ids)

    if is_qwen3vl:
        # For Qwen3-VL videos, the HF processor pre-expands one video into
        # K per-frame <vision_start>...<vision_end> spans (K = grid_thw[0]).
        # Collapse all consecutive video spans into a single <vs, vp, ve> first.
        vs_id = processor.tokenizer.convert_tokens_to_ids("<|vision_start|>")
        ve_id = processor.tokenizer.convert_tokens_to_ids("<|vision_end|>")
        vp_id = processor.video_token_id
        vs_pos = np.where(arr == vs_id)[0]
        ve_pos = np.where(arr == ve_id)[0]
        if len(vs_pos) > 0 and len(vs_pos) == len(ve_pos):
            is_video_block = [
                bool(np.any(arr[s + 1 : e] == vp_id))
                for s, e in zip(vs_pos, ve_pos)
            ]
            if any(is_video_block):
                first_vid = is_video_block.index(True)
                last_vid = len(is_video_block) - 1 - is_video_block[::-1].index(True)
                head = arr[: vs_pos[first_vid]].tolist()
                tail = arr[ve_pos[last_vid] + 1 :].tolist()
                arr = np.array(head + [int(vs_id), int(vp_id), int(ve_id)] + tail)

    # Standard dedup: collapse consecutive image_pad / video_pad runs to one.
    # Needed for Qwen2/2.5-VL and for Qwen3-VL image-only prompts (where the HF
    # processor expands <|image_pad|> in-place so consecutive pads need dedup).
    mask = np.ones(len(arr), dtype=bool)
    is_value = (arr == processor.image_token_id) | (arr == processor.video_token_id)
