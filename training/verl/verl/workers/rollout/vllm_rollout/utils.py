...
class vLLMColocateWorkerExtension:
    ...
    def collect_exploration_stats(self) -> dict:
        """Snapshot and reset entropy-aware-exploration LP counters in this
        worker subprocess. Counters live in module-level state inside the
        worker process; the Ray actor cannot read them directly. This method
        is invoked via ``engine.collective_rpc`` so the actor receives the
        per-worker snapshots and aggregates them.

        Safe to call when exploration is disabled (returns zero counters).
        """
        try:
            from verl.workers.rollout.exploration.entropy_dropout_lp import (
                flush_lp_counters,
            )

            return flush_lp_counters()
        except Exception:
            return {}
