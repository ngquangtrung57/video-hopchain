...
class vLLMHttpServer:
    ...
    async def launch_server(self, master_address: str = None, master_port: int = None, dp_rpc_port: int = None):
        ...
        # Entropy-aware exploration: propagate config to the logits processor
        # and register the processor class with the vLLM engine. Zero effect
        # when `exploration.enable` is False.
        exploration_cfg = getattr(self.config, "exploration", None)
        if exploration_cfg is not None and exploration_cfg.enable:
            from verl.workers.rollout.exploration import entropy_dropout_lp as _edlp

            _exp_cfg_dict = {
                "enable": True,
                "trigger_mode": getattr(exploration_cfg, "trigger_mode", "high"),
                "top_prob_threshold": exploration_cfg.top_prob_threshold,
                "tau_low": getattr(exploration_cfg, "tau_low", 0.3),
                "tau_high": getattr(exploration_cfg, "tau_high", 0.7),
                "perturb_prob": exploration_cfg.perturb_prob,
                "drop_top_k": exploration_cfg.drop_top_k,
                "min_position": exploration_cfg.min_position,
                "max_perturbations_per_seq": exploration_cfg.max_perturbations_per_seq,
                "restrict_to_think_region": exploration_cfg.restrict_to_think_region,
                "think_open_token_id": exploration_cfg.think_open_token_id,
                "think_close_token_id": exploration_cfg.think_close_token_id,
                "seed": exploration_cfg.seed,
                # Every key here must be threaded to the vLLM workers via env
                # var so the LP can recover it inside the spawned process.
                "deterministic": getattr(exploration_cfg, "deterministic", False),
                "mask_from_loss": getattr(exploration_cfg, "mask_from_loss", False),
                "prompt_exploration_prob": getattr(exploration_cfg, "prompt_exploration_prob", 1.0),
                "think_prefilled": bool(
                    getattr(exploration_cfg, "think_prefilled", False)
                    or os.environ.get("VERL_THINK_PREFILL", "0") == "1"
                ),
                "record_details": getattr(exploration_cfg, "record_details", False),
                "drop_trace_dir": str(getattr(exploration_cfg, "drop_trace_dir", "") or ""),
                "drop_trace_max_rows": int(getattr(exploration_cfg, "drop_trace_max_rows", 20000)),
                "skip_special_tokens": bool(getattr(exploration_cfg, "skip_special_tokens", True)),
                "skip_whitespace_punct": bool(getattr(exploration_cfg, "skip_whitespace_punct", False)),
                "skip_digit_tokens": bool(getattr(exploration_cfg, "skip_digit_tokens", False)),
                "skip_subword_continuation": bool(getattr(exploration_cfg, "skip_subword_continuation", False)),
                "counter_snapshot_every": int(getattr(exploration_cfg, "counter_snapshot_every", 2000)),
                "record_entropy": getattr(exploration_cfg, "record_entropy", True),
                "detail_topn": getattr(exploration_cfg, "detail_topn", 8),
                "detail_max_per_request": getattr(exploration_cfg, "detail_max_per_request", 512),
                # This dict is the only channel to the spawned vLLM worker: a
                # key missing here leaves that knob pinned to its default with
                # no error. test_worker_config_ships_every_key_the_lp_reads
                # guards the whole set.
                "measure_entropy": bool(getattr(exploration_cfg, "measure_entropy", True)),
                "entropy_measure_stride": int(getattr(exploration_cfg, "entropy_measure_stride", 1)),
                "entropy_post_drop_window": int(getattr(exploration_cfg, "entropy_post_drop_window", 16)),
                "entropy_seq_trace_max_rows": int(
                    getattr(exploration_cfg, "entropy_seq_trace_max_rows", 40000)
                ),
            }
            _edlp.set_exploration_config(_exp_cfg_dict)
            # vLLM forces spawn for worker subprocesses inside Ray actors,
            # which reimports the LP module fresh and discards our in-memory
            # config. Mirror it through the environment so the LP's __init__
            # can recover it on the worker side.
            import json as _json
            os.environ["VERL_EXPLORATION_CFG"] = _json.dumps(_exp_cfg_dict)
            lps = list(engine_kwargs.get("logits_processors", []) or [])
            lps.append(
                "verl.workers.rollout.exploration.entropy_dropout_lp:EntropyDropoutLP"
            )
            engine_kwargs["logits_processors"] = lps
            _trig_mode = getattr(exploration_cfg, "trigger_mode", "high")
            if _trig_mode == "band":
                logger.info(
                    "rvrl exploration enabled (mode=band, k_explore=%d, "
                    "tau_low=%.2f, tau_high=%.2f, p=%.2f, "
                    "restrict_to_think=%s, drop_top_k=%d)",
                    exploration_cfg.k_explore,
                    getattr(exploration_cfg, "tau_low", 0.3),
                    getattr(exploration_cfg, "tau_high", 0.7),
                    exploration_cfg.perturb_prob,
                    exploration_cfg.restrict_to_think_region,
                    exploration_cfg.drop_top_k,
                )
            else:
                logger.info(
                    "rvrl exploration enabled (mode=high, k_explore=%d, tau=%.2f, p=%.2f, "
                    "restrict_to_think=%s, drop_top_k=%d)",
                    exploration_cfg.k_explore,
                    exploration_cfg.top_prob_threshold,
                    exploration_cfg.perturb_prob,
                    exploration_cfg.restrict_to_think_region,
                    exploration_cfg.drop_top_k,
                )
    ...
    async def generate(
        ...
        # When video tensors are pre-fetched by qwen_vl_utils (already at the
        # target frame count), forbid vLLM's processor from re-sampling so its
        # placeholder count matches the prompt_ids tokenized by the HF
        # processor; otherwise masked_scatter in the qwen3_vl forward asserts.
        if video_data is not None:
        ...
        # For exploration requests, drain the per-request dropped-position list
        # from the LP side-channel and pack it into extra_fields. Empty list
        # when the LP never triggered; other code paths see a missing key and
        # treat it as no drops.
        extra_args = getattr(sampling_params, "extra_args", None)
        if extra_args and extra_args.get("rvrl_exploration"):
            req_uuid = extra_args.get("rvrl_req_uuid")
            if req_uuid:
                from verl.workers.rollout.exploration import consume_dropped_positions
                extra_fields["exploration_drop_positions"] = consume_dropped_positions(str(req_uuid))
                # Drain the parallel analysis record. A separate table popped
                # separately, so a failure here cannot disturb the loss-mask
                # contract above. Absent or empty when record_details is off.
                try:
                    from verl.workers.rollout.exploration import consume_dropped_details
                    _det = consume_dropped_details(str(req_uuid))
                    if _det:
                        extra_fields["exploration_drop_details"] = _det
                except Exception:
                    pass
    ...
    async def get_exploration_stats(self) -> dict:
        """Snapshot and reset entropy-aware exploration LP counters.

        The LP runs inside vLLM worker subprocesses (not in this Ray actor's
        process). Counters live in module-level state in those workers and
        are not visible from the actor's namespace. We therefore use
        ``engine.collective_rpc`` to invoke ``collect_exploration_stats`` on
        the worker extension class, then aggregate snapshots across workers.

        Safe to call when exploration is disabled (returns zero counters).
        Called by the trainer once per param-sync step to assemble
        ``rvrl/lp_*`` W&B metrics.
        """
        try:
            results = await self.engine.collective_rpc(
                "collect_exploration_stats"
            )
            from verl.workers.rollout.exploration import merge_lp_counters
            agg: dict = {}
            for snap in results or []:
                merge_lp_counters(agg, snap)
            return agg
        except Exception as e:
            logger.warning(f"get_exploration_stats failed: {e}", exc_info=True)
            return {}
