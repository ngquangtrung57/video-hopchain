...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def init_model(self):
        ...
        # Honor data.max_pixels on the worker-side processor too. The driver applies
        # the same cap in main_ppo (auto-merged from 239bb9e6); replicate here in case
        # the worker's processor is consulted for any image-processor pass on rollout
        # or response side. Defensive .get because actor_rollout_ref config may not
        # carry the global data subtree in the new engine_workers split.
        _data_cfg = self.config.get("data", None) if hasattr(self.config, "get") else None
        if (
            _data_cfg is not None
            and model_config.processor is not None
            and hasattr(model_config.processor, "image_processor")
        ):
            _max_pixels = _data_cfg.get("max_pixels", None)
            if _max_pixels is not None:
                model_config.processor.image_processor.max_pixels = int(_max_pixels)
