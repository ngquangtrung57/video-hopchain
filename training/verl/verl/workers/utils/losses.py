...
def ppo_loss(config: ActorConfig, model_output, data: TensorDict, dp_group=None):
    ...
    # Thread exploration_drop_mask through the to-padded conversion when present.
    has_drop_mask = "exploration_drop_mask" in data
    if has_drop_mask:
        fields.append("exploration_drop_mask")
    ...
    # Build effective_mask for PG aggregation. Exploration-dropped positions
    # (forced resamples) are excluded from the policy gradient: they are not
    # the policy's own choice. response_mask itself is kept intact so
    # entropy/KL/length metrics still cover the full response.
    if has_drop_mask:
        drop_mask = data["exploration_drop_mask"].to(bool)
        if drop_mask.shape == response_mask.shape:
            effective_mask = response_mask & ~drop_mask
        else:
